...
from .model import Continuity, Document, SearchRequest, SearchResponse, ApplicationBiblio, Application, AssigneeAddress, Assignment, Transaction, ForeignPriority, CustomerNumber, TermAdjustment, TermAdjustmentHistory
...
def test_continuity(fixture_dir):
    ...
    assert continuity.parent_continuity[0].application_status_description == "Abandoned  --  Failure to Respond to an Office Action"
    ...
    assert continuity.child_continuity[0].application_status_description == "Abandoned  --  Incomplete Application (Pre-examination)"
...
def test_document(fixture_dir):
    ...
    for doc_data in data["documentBag"]:
        ...
        for download_option in document.download_option_bag:
            ...
            assert download_option["downloadUrl"].startswith("https://beta-api.uspto.gov/api/v1/download/applications/16330077/")
...
def test_transactions(fixture_dir):
    ...
    transactions = [Transaction(**transaction_data) for transaction_data in data["patentBag"][0]["transactionContentBag"]]
    ...
    assert transactions[3].transaction_description == "Applicant Has Filed a Verified Statement of Small Entity Status in Compliance with 37 CFR 1.27"
...
def test_application_biblio(fixture_dir):
    ...
    application_biblio = ApplicationBiblio(**application_biblio_data)
...
def test_search_response_and_result(fixture_dir):
    data = json.loads((fixture_dir / "search.json").read_text())
    search_response_data = data
    search_response = SearchResponse(**search_response_data)

    assert search_response.count > 0
    assert search_response.request_id is not None
    assert len(search_response.results) > 0

    for result in search_response.results:
        assert result.filing_date is not None
        assert result.appl_id is not None
        assert result.invention_title is not None
