...
class PublishedBiblioApi:
    ...
    @classmethod
    async def get_biblio(cls, number: str, doc_type: str = "publication", format: str = "docdb") -> ET._Element:
    ...
    @classmethod
    @function_cache
    async def get_abstract(cls, number: str, doc_type: str = "publication", format: str = "docdb") -> ET._Element:
    ...
    @classmethod
    @function_cache
    async def get_full_cycle(cls, number: str, doc_type: str = "publication", format: str = "docdb") -> ET._Element:
...
class PublishedFulltextApi:
    ...
    @classmethod
    @function_cache
    async def get_description(cls, number, doc_type="publication", format="docdb") -> ET._Element:
        return await cls.get_fulltext_result(number, doc_type="publication", format="docdb", inquiry="description")

    @classmethod
    @function_cache
    async def get_claims(cls, number, doc_type="publication", format="docdb") -> ET._Element:
...
class PublishedSearchApi:
    ...
    @classmethod
    @function_cache
    async def search(cls, query: str, start: int = 1, end: int = 100) -> ET._Element:
        base_url = "http://ops.epo.org/3.2/rest-services/published-data/search"
        range = f"{start}-{end}"
        logger.debug(f"OPS Search Endpoint - Query: {query}\nRange: {start}-{end}")
        response = await cls.http_client.get(base_url, params={"Range": range, "q": query})
        return ET.fromstring(response.content)
...
class PublishedImagesApi:
    ...
    @classmethod
    @function_cache
    async def get_images(cls, number: str, doc_type: str = "publication", format: str = "docdb") -> ET._Element:
        base_url = f"http://ops.epo.org/3.2/rest-services/published-data/{doc_type}/{format}/{number}/images"
        response = await cls.http_client.get(base_url)
        return ET.fromstring(response.content)
    ...
    async def get_page_image(
        ...
        return await cls.http_client.download(url, method="GET", params={"Range": page_number}, path=path)

    @classmethod
    @function_cache
    async def get_page_image_from_link(
        cls,
        link,
        page_number,
        image_format="pdf",
