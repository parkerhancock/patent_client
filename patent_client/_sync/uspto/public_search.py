...
class PublicSearchAsyncApi:
    ...
    def query(
        ...
        query_response = cls.http_client.post(url, json=data)
    ...
    @classmethod
    @function_cache
    def get_document(cls, guid: str, source: str) -> tp.Any:
        ...
        response = cls.http_client.get(url, params=params)
        response.raise_for_status()
        return response.json()

    @classmethod
    def _get_session(cls) -> None:
        url = "https://ppubs.uspto.gov/dirsearch-public/users/me/session"
        response = cls.http_client.post(url, json=-1)  # json=str(random.randint(10000, 99999)))
        cls.session = response.json()
        cls.case_id = cls.session["userCase"]["caseId"]

    @classmethod
    def _request_save(self, guid, image_location, source, page_count) -> str:
