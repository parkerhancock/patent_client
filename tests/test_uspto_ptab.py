...
class TestPtab:
    ...
    def test_can_get_set_of_cases(self):
        cases = PtabTrial.objects.filter(
            patent_owner_name="Silicon Genesis Corporation"
        ).order_by("trial_number")
        print(cases.params)
        assert cases.count() == 3
        assert list(cases)[0].trial_number == "IPR2016-00831"
    ...
    def test_can_get_case_documents(self):
        ...
        assert (
            case.documents.first().title
            == "U.S. Provisional Application No. 60/046,276"
        )
        assert list(case.documents.values_list("title", flat=True))[:5] == [
            "U.S. Provisional Application No. 60/046,276",
            "U.S. Patent No. 5,374,564",
            "L. Wegmann, Historical Perspective and Future Trends for Ion Implantation Systems (1981)",
            "U.S. Patent No. 5,559,043",
            "Prosecution History of U.S. Patent No. 6,013,563",
        ]

    def test_can_get_case_from_document(self):
        doc = PtabDocument.objects.get(id=230910)
