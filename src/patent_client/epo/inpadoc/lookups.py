...
def docid_to_inpadoc(doc, model_name="Inpadoc"):
    klass = getattr(
        importlib.import_module("patent_client.epo.inpadoc.model"), model_name
    )
...
def parse_family_member(member):
    family_class = getattr(
        importlib.import_module("patent_client.epo.inpadoc.model"),
        "InpadocFamilyMember",
    )
    priority_claim_class = getattr(
        importlib.import_module("patent_client.epo.inpadoc.model"),
        "InpadocFamilyPriorityClaim",
    )
    pub = member.find(
        './/ex:publication-reference/ex:document-id[@document-id-type="docdb"]', NS
    )
    app = member.find(
        './/ex:application-reference/ex:document-id[@document-id-type="docdb"]', NS
    )
    family_id = int(member.attrib["family-id"])
    priority = member.findall(".//ex:priority-claim", NS)
    priority_claims = list()
    for c in priority:
        doc = docid_to_inpadoc(
            c.find('.//ex:document-id[@document-id-type="docdb"]', NS), "Inpadoc"
        )
