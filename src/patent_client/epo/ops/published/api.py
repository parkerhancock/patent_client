...
from .model import Search, BiblioResult, Claims, Description, Images
from .schema import BiblioResultSchema, SearchSchema, DescriptionSchema, ClaimsSchema, ImagesSchema
...
class PublishedFulltextApi():
    ...
    def get_fulltext_result(cls, number, doc_type="publication", format="docdb", inquiry="fulltext"):
        ...
        if number[:2] not in cls.fulltext_jurisdictions:
            raise ValueError(f"Fulltext Is Not Available For Country Code {number[:2]}. Fulltext is only available in {', '.join(cls.fulltext_jurisdictions)}")
        response = session.get(url)
        response.raise_for_status
